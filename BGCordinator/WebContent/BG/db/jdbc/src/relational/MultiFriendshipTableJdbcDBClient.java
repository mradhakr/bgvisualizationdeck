...
public class MultiFriendshipTableJdbcDBClient extends DB implements JdbcDBClientConstants {
	...
	public int viewProfile(int requesterID, int profileOwnerID,
		...
		try {
			//friend count
			query = "SELECT count(*) FROM  friendship WHERE friend1 = ? OR friend2 = ?";
			//preparedStatement = conn.prepareStatement(query);
		...
		if(requesterID == profileOwnerID){
			query = "SELECT count(*) FROM  pendingfriendship WHERE inviteeID = ?";
			try {
				//preparedStatement = conn.prepareStatement(query);
	...
	public int listFriends(int requesterID, int profileOwnerID,
		...
		try {
			if(insertImage && FSimagePath.equals("")){
				query = "SELECT userid,friend1, friend2, username, fname, lname, gender, dob, jdate, ldate, address,email,tel,tpic FROM users, friendship WHERE ((friend1=? and userid=friend2) or (friend2=? and userid=friend1))";
				...
				query = "SELECT userid,friend1, friend2, username, fname, lname, gender, dob, jdate, ldate, address,email,tel FROM users, friendship WHERE ((friend1=? and userid=friend2) or (friend2=? and userid=friend1))";
	...
	public int rejectFriend(int inviterID, int inviteeID) {
		...
		String query = "DELETE FROM pendingfriendship WHERE inviterid=? and inviteeid= ? ";
		try {
	...
	public int CreateFriendship(int memberA, int memberB) {
		...
		if(memberA < 0 || memberB < 0)
			return -1;
		try {
			String DML = "INSERT INTO friendship values(?,?)";
	...
	public int inviteFriend(int inviterID, int inviteeID) {
		...
		if(inviterID < 0 || inviteeID < 0)
			return -1;
		String query = "INSERT INTO pendingfriendship values(?,?)";
		try {
	...
	public int thawFriendship(int friendid1, int friendid2){
		...
		String query = "DELETE FROM friendship WHERE (friend1=? and friend2= ?) OR (friend2=? and friend1= ?)";
		try {
	...
	public HashMap<String, String> getInitialStats() {
		...
		try {
			...
			query = "select count(*) from friendship where (friend1="+Integer.parseInt(offset) +" OR friend2="+Integer.parseInt(offset) +")" ;
			rs = st.executeQuery(query);
			if(rs.next()){
			...
			if(rs != null) rs.close();
			query = "select count(*) from pendingfriendship where (inviteeid="+Integer.parseInt(offset) +")" ;
			rs = st.executeQuery(query);
			if(rs.next()){
	...
	public int queryConfirmedFriendshipIds(int profileId, Vector<Integer> confirmedIds){
		...
		try {
			st = conn.createStatement();
			query = "SELECT friend1, friend2 from friendship where (friend1="+profileId+" OR friend2="+profileId+")";
			rs = st.executeQuery(query);
			while(rs.next()){
	...
	public void createSchema(Properties props){
		...
		try {
			...
			stmt.executeUpdate("ALTER TABLE FRIENDSHIP ADD CONSTRAINT FRIENDSHIP_PK PRIMARY KEY (FRIEND1, FRIEND2) ENABLE");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIP MODIFY (FRIEND1 NOT NULL ENABLE)");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIP MODIFY (FRIEND2 NOT NULL ENABLE)");
			...
			stmt.executeUpdate("ALTER TABLE FRIENDSHIP ADD CONSTRAINT FRIENDSHIP_USERS_FK1 FOREIGN KEY (FRIEND1)"
					+ "REFERENCES USERS (USERID) ON DELETE CASCADE ENABLE");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIP ADD CONSTRAINT FRIENDSHIP_USERS_FK2 FOREIGN KEY (FRIEND2)"
					+ "REFERENCES USERS (USERID) ON DELETE CASCADE ENABLE");
			...
			dropIndex(stmt, "FRIENDSHIP_FRIEND1");
			dropIndex(stmt, "FRIENDSHIP_FRIEND2");
	...
	public void buildIndexes(Properties props){
		...
		try {
			...
			stmt.executeUpdate("CREATE INDEX FRIENDSHIP_FRIEND1 ON FRIENDSHIP (FRIEND1)"
					+ "COMPUTE STATISTICS NOLOGGING");
			...
			stmt.executeUpdate("CREATE INDEX FRIENDSHIP_FRIEND2 ON FRIENDSHIP (FRIEND2)"
					+ "COMPUTE STATISTICS NOLOGGING");
